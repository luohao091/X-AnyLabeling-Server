...
from sam3.model.sam3_tracker_base import (
    concat_points,
    NO_OBJ_SCORE,
    Sam3TrackerBase,
)
...
class Sam3TrackerPredictor(Sam3TrackerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.always_start_from_first_ann_frame = (
    ...
    def add_new_mask(
        ...
        for (
            obj_idx2,
            obj_temp_output_dict2,
        ) in temp_output_dict_per_obj.items():
            if obj_idx2 == obj_idx:
                continue
            current_out2 = obj_temp_output_dict2[storage_key].get(
                frame_idx, None
            )
            if (
                current_out2 is not None
                and "pred_masks_video_res" in current_out2
            ):
    ...
    def _consolidate_temp_output_across_obj(
        ...
        if consolidate_at_video_res:
            assert (
                not run_mem_encoder
            ), "memory encoder cannot run at video resolution"
        ...
        for obj_idx in range(batch_size):
            ...
            if self.use_memory_selection:
                consolidated_out["iou_score"][obj_idx : obj_idx + 1] = out[
                    "iou_score"
                ]
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(
            ...
            if frame_idx in consolidated_frame_inds["cond_frame_outputs"]:
                ...
                frame_idx in consolidated_frame_inds["non_cond_frame_outputs"]
            ):
    ...
    def clear_all_points_in_frame(
        ...
        if not frame_has_input:
            ...
            for obj_idx2 in range(batch_size):
                ...
                obj_out = obj_output_dict["cond_frame_outputs"].pop(
                    frame_idx, None
                )
    ...
    def _run_memory_encoder(
        ...
        """
        # Retrieve correct image features
        image, _, current_vision_feats, _, feat_sizes = (
            self._get_image_feature(inference_state, frame_idx, batch_size)
        )
    ...
    def _suppress_object_pw_area_shrinkage(self, pred_masks):
        ...
        pixel_level_non_overlapping_masks = (
            super()._apply_non_overlapping_constraints(pred_masks)
        )
    ...
    def _apply_object_wise_non_overlapping_constraints(
        ...
        # Apply pixel-wise non-overlapping constraint based on mask scores
        pixel_level_non_overlapping_masks = (
            super()._apply_non_overlapping_constraints(pred_masks_single_score)
        )
