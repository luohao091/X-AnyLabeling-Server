...
class Sam3VideoPredictorMultiGPU(Sam3VideoPredictor):
    def __init__(self, *model_args, gpus_to_use=None, **model_kwargs):
        ...
        if IS_MAIN_PROCESS:
            ...
            assert len(gpus_to_use) > 0 and all(
                isinstance(i, int) for i in gpus_to_use
            )
    ...
    def _start_worker_processes(self, *model_args, **model_kwargs):
        ...
        self.command_queues = {
            rank: mp_ctx.Queue() for rank in range(1, world_size)
        }
        self.result_queues = {
            rank: mp_ctx.Queue() for rank in range(1, world_size)
        }
        ...
        for rank in range(1, world_size):
            ...
            os.environ["IS_MAIN_PROCESS"] = (
                "0"  # mark this as a worker process
            )
    ...
    def shutdown(self):
        ...
        if self.rank == 0 and self.world_size > 1:
            ...
            for rank in range(1, self.world_size):
                self.result_queues[
                    rank
                ].get()  # wait for the worker to acknowledge
