...
class Sam3VideoBase(nn.Module):
    def __init__(
        ...
        self._dist_pg_cpu = (
            None  # CPU process group (lazy-initialized on first use)
        )
    ...
    def device(self):
        self._device = (
            getattr(self, "_device", None) or next(self.parameters()).device
        )
        return self._device
    ...
    def broadcast_python_obj_cpu(self, python_obj_list, src):
        ...
        dist.broadcast_object_list(
            python_obj_list, src=src, group=self._dist_pg_cpu
        )
    ...
    def _det_track_one_frame(
        ...
        frame_stats = {
            ...
            "num_obj_dropped": tracker_update_plan[
        ...
        if tracker_obj_scores_global.shape[0] > 0:
            ...
            tracker_obj_scores_global = (
                tracker_obj_scores_global.sigmoid().tolist()
            )
    ...
    def run_backbone_and_detection(
        ...
        start_frame_idx = tracking_bounds.get(
        ...
        feature_cache.pop(
            frame_idx - 1 if not reverse else frame_idx + 1, None
        )
        return det_out
    ...
    def run_tracker_propagation(
        ...
        assert np.all(
            obj_ids_local
            == tracker_metadata_prev["obj_ids_per_gpu"][self.rank]
        ), "{} != {}".format(
        ...
        if self.world_size > 1:
            ...
            num_obj_this_gpu = tracker_metadata_prev["num_obj_per_gpu"][
                self.rank
            ]
    ...
    def _recondition_masklets(
        ...
        for trk_obj_id, det_idx in trk_id_to_max_iou_high_conf_det.items():
            ...
            obj_idx = np.where(
                tracker_metadata["obj_ids_all_gpu"] == trk_obj_id
            )[0].item()
    ...
    def run_tracker_update_planning_phase(
        ...
        if self.rank == 0:
            ...
            if (
                not is_image_only
                and prev_obj_num + new_det_num > self.max_num_objects
            ):
                logger.warning(
                ...
                num_obj_dropped_due_to_limit = (
                    new_det_num - new_det_num_to_keep
                )
            ...
            rank0_metadata_new = deepcopy(
                tracker_metadata_prev["rank0_metadata"]
            )
            if (
                not hasattr(self, "_warm_up_complete")
                or self._warm_up_complete
            ):
                obj_ids_newly_removed, rank0_metadata_new = (
                    self._process_hotstart(
                        frame_idx=frame_idx,
                        num_frames=num_frames,
                        reverse=reverse,
                        det_to_matched_trk_obj_ids=det_to_matched_trk_obj_ids,
        ...
        for rank in range(self.world_size):
            ...
            if len(obj_ids_newly_removed) > 0:
                ...
                updated_obj_ids_this_gpu = updated_obj_ids_this_gpu[
                    ~is_removed
                ]
        ...
        if self.rank == 0 and self.masklet_confirmation_enable:
            rank0_metadata = self.update_masklet_confirmation_status(
                ...
                obj_ids_all_gpu_updated=tracker_metadata_new[
                    "obj_ids_all_gpu"
                ],
                det_to_matched_trk_obj_ids=det_to_matched_trk_obj_ids,
                new_det_obj_ids=new_det_obj_ids,
    ...
    def _suppress_overlapping_based_on_recent_occlusion(
        ...
        if batch_size > 0:
            ...
            to_suppress = (
                self._get_objects_to_suppress_based_on_most_recently_occluded(
            ...
            is_obj_occluded = ~(
                binary_tracker_low_res_masks_global.any(dim=(-1, -2))
            )
    ...
    def run_tracker_update_execution_phase(
        ...
        obj_ids_newly_removed: Set[int] = tracker_update_plan[
            "obj_ids_newly_removed"
        ]
        ...
        if len(obj_ids_newly_removed) > 0:
            self._tracker_remove_objects(
                tracker_states_local, obj_ids_newly_removed
            )
    ...
    def build_outputs(
        ...
        for obj_id, mask in zip(
            existing_masklet_obj_ids, existing_masklet_binary
        ):
        ...
        if (
            reconditioned_obj_ids is not None
            and len(reconditioned_obj_ids) > 0
        ):
    ...
    def _associate_det_trk(
        ...
        assert (
            det_masks.is_floating_point()
        ), "float tensor expected (do not binarize)"
        assert (
            trk_masks.is_floating_point()
        ), "float tensor expected (do not binarize)"
        ...
        trk_id_to_max_iou_high_conf_det = (
            {}
        )  # trk id --> exactly one detection idx
        HIGH_CONF_THRESH = 0.8
        HIGH_IOU_THRESH = 0.8
        ...
        for d in range(det_masks.size(0)):
            det_to_matched_trk_obj_ids[d] = trk_obj_ids[
                ious_np[d, :] >= iou_threshold
            ]
    ...
    def _process_hotstart(
        ...
        overlap_pair_to_frame_inds = rank0_metadata[
        ...
        obj_ids_newly_removed = (
            set()
        )  # object IDs to be newly removed on this frame
        hotstart_diff = (
        ...
        for _, matched_trk_obj_ids in det_to_matched_trk_obj_ids.items():
            ...
            first_appear_obj_id = (
                ...
                if not reverse
                    ...
                    matched_trk_obj_ids, key=lambda x: obj_first_frame_idx[x]
                )
            )
        ...
        for (
            first_obj_id,
            obj_id,
        ), frame_indices in overlap_pair_to_frame_inds.items():
            if obj_id in removed_obj_ids or obj_id in obj_ids_newly_removed:
                continue  # skip if the object is already removed
            if (
                obj_first_frame_idx[obj_id] > hotstart_diff and not reverse
    ...
    def _tracker_update_memories(
        ...
        for tracker_state in tracker_inference_states:
            ...
            local_high_res_masks = high_res_masks[
    ...
    def _tracker_remove_object(
        self, tracker_states_local: List[Any], obj_id: int
    ):
        """
    ...
    def _initialize_metadata(self):
        ...
        tracker_metadata = {
            "obj_ids_per_gpu": [
                np.array([], np.int64) for _ in range(self.world_size)
            ],
    ...
    def update_masklet_confirmation_status(
        ...
        prev_elem_is_in_updated = np.isin(
            obj_ids_all_gpu_prev, obj_ids_all_gpu_updated
        )
        prev_elem_obj_ids_in_updated = obj_ids_all_gpu_prev[
            prev_elem_is_in_updated
        ]
        prev_elem_inds_in_updated = np.array(
            [
                obj_id_to_updated_idx[obj_id]
                for obj_id in prev_elem_obj_ids_in_updated
            ],
            dtype=np.int64,
        )
        ...
        status = np.full_like(
            obj_ids_all_gpu_updated, fill_value=unconfirmed_val
        )
        status[prev_elem_inds_in_updated] = status_prev[
            prev_elem_is_in_updated
        ]
        consecutive_det_num = np.zeros_like(obj_ids_all_gpu_updated)
        consecutive_det_num[prev_elem_inds_in_updated] = (
    ...
    def forward(self, input: BatchedDatapoint, is_inference: bool = False):
        raise NotImplementedError(
            "Evaluation outside demo is not implemented yet"
        )
    ...
    def _load_checkpoint(self, ckpt_path: str, strict: bool = True):
        ...
        if len(missing_keys) > 0 or len(unexpected_keys) > 0:
            logger.warning(
                f"Loaded ckpt with {missing_keys=}, {unexpected_keys=}"
            )
        else:
            logger.info(
                "Loaded ckpt successfully without missing or unexpected keys"
            )
    ...
    def _drop_new_det_with_obj_limit(
        self, new_det_fa_inds, det_scores_np, num_to_keep
    ):
        """
