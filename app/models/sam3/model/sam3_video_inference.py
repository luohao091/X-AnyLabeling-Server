...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def reset_state(self, inference_state):
        ...
        inference_state["input_batch"].find_text_batch[
            0
        ] = "<text placeholder>"
    ...
    def _get_visual_prompt(
        self, inference_state, frame_idx, boxes_cxcywh, box_labels
    ):
        """
    ...
    def _get_processing_order(
        ...
        if (
            all(out is None for out in previous_stages_out)
            and start_frame_idx is None
        ):
            raise RuntimeError(
        ...
        if start_frame_idx is None:
            ...
            start_frame_idx = min(
                t
                for t, out in enumerate(previous_stages_out)
                if out is not None
            )
            ...
            if self.hotstart_delay > 0:
                ...
                if self.rank == 0:
                    ...
                    if unconfirmed_obj_ids is not None:
                        unconfirmed_obj_ids_per_frame[frame_idx] = (
                            unconfirmed_obj_ids
                        )
    ...
    def _run_single_frame_inference(self, inference_state, frame_idx, reverse):
        ...
        if self.rank == 0:
            ...
            if self.masklet_confirmation_enable:
                ...
                is_unconfirmed = (
                    status == MaskletConfirmationStatus.UNCONFIRMED.value
                )
                out["unconfirmed_obj_ids"] = tracker_metadata_new[
                    "obj_ids_all_gpu"
                ][is_unconfirmed].tolist()
            else:
    ...
    def _postprocess_output(
        ...
        if len(curr_obj_ids) == 0:
            ...
            out_tracker_probs = torch.index_select(
                out_tracker_probs, 0, keep_idx
            )
            out_binary_masks = torch.index_select(
                out_binary_masks, 0, keep_idx_gpu
            )
    ...
    def add_fake_objects_to_inference_state(
        ...
        inference_state["tracker_inference_states"] = (
            self._tracker_add_new_objects(
                ...
                tracker_states_local=inference_state[
        ...
        if num_objects > 0:
            ...
            for i, obj_id in enumerate(new_det_obj_ids_local):
                obj_id_to_mask[obj_id] = (video_res_masks[i] > 0.0).to(
                    torch.bool
                )
        if self.rank == 0:
    ...
    def add_prompt(
        ...
        if boxes_xywh is not None:
            ...
            assert box_labels.dim() == 1 and box_labels.size(
                0
            ) == boxes_xywh.size(0)
            ...
            boxes_cxcywh, box_labels, geometric_prompt = (
                self._get_visual_prompt(
    ...
    def forward(self, input: BatchedDatapoint, is_inference: bool = False):
        ...
        for prompt_id, prompt in zip(text_prompt_ids, text_prompt_list):
            ...
            start_obj_id = (
                max(scores_labels.keys(), default=-1) + 1
            )  # prev max + 1
...
class Sam3VideoInferenceWithInstanceInteractivity(Sam3VideoInference):
    ...
    def propagate_in_video(
        ...
        if propagation_type == "propagation_partial":
            ...
            tracker_states_local = (
                self._get_tracker_inference_states_by_obj_ids(
        ...
        if "propagation" in action_history[-1]["type"]:
            if action_history[-1]["type"] in ["propagation_fetch"]:
                ...
                if (
                    ...
                    return (
                        action_history[-1]["type"],
                        action_history[-1]["obj_ids"],
                    )
    ...
    def remove_object(self, inference_state, obj_id, is_user_action=False):
        ...
        tracker_metadata["obj_ids_per_gpu"][obj_rank] = _obj_ids[
            _obj_ids != obj_id
        ]
        ...
        if "cached_frame_outputs" in inference_state:
            for frame_idx in inference_state["cached_frame_outputs"]:
                frame_cache = inference_state["cached_frame_outputs"][
                    frame_idx
                ]
    ...
    def _get_gpu_id_by_obj_id(self, inference_state, obj_id):
        ...
        """
        obj_ids_per_gpu = inference_state["tracker_metadata"][
            "obj_ids_per_gpu"
        ]
    ...
    def _get_tracker_inference_states_by_obj_ids(
    ...
    def add_tracker_new_points(
        ...
        tracker_metadata["obj_id_to_tracker_score_frame_wise"][frame_idx][
            obj_id
        ] = 1.0
        ...
        if self.rank == 0:
            ...
            if "masklet_confirmation" in rank0_metadata:
                ...
                if len(obj_indices) > 0:
                    ...
                    if obj_idx < len(
                        rank0_metadata["masklet_confirmation"]["status"]
                    ):
                        rank0_metadata["masklet_confirmation"]["status"][
                            obj_idx
                        ] = 1
    ...
    def _gather_obj_id_to_mask_across_gpus(
        self, inference_state, obj_id_to_mask_local
    ):
    ...
    def _convert_low_res_mask_to_video_res(
        self, low_res_mask, inference_state
    ):
        """
