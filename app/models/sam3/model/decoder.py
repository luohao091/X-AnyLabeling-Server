...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        if use_text_cross_attention:
            self.ca_text = nn.MultiheadAttention(
    ...
    def forward(
        ...
        tgt_query_pos: Optional[
            Tensor
        ] = None,  # pos for query. MLP(Sine(pos))
        tgt_query_sine_embed: Optional[
            Tensor
        ] = None,  # pos for query. Sine(pos)
        ...
        self_attn_mask: Optional[
            Tensor
        ] = None,  # mask used for self-attention
        cross_attn_mask: Optional[
            Tensor
        ] = None,  # mask used for cross-attention
        # dac
        dac=False,
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.instance_norm = (
            nn.LayerNorm(d_model) if separate_norm_instance else None
        )
        ...
        self.roi_pooler = (
            RoIAlign(
                output_size=7, spatial_scale=1, sampling_ratio=-1, aligned=True
            )
            if interaction_layer is not None
            else None
        )
        ...
        self.ref_point_head = MLP(
            2 * self.d_model, self.d_model, self.d_model, 2
        )
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        if (
            ...
            or self.compilable_stored_size
            == (
                H,
                W,
            )
        ...
        deltas_y = (
            coords_h.view(1, -1, 1) - boxes_xyxy.reshape(-1, 1, 4)[:, :, 1:4:2]
        )
        deltas_y = deltas_y.view(bs, num_queries, -1, 2)
        deltas_x = (
            coords_w.view(1, -1, 1) - boxes_xyxy.reshape(-1, 1, 4)[:, :, 0:3:2]
        )
        ...
        if self.training:
            ...
            ), "activation ckpt not enabled in decoder"
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if self.boxRPB != "none" and reference_boxes is not None:
                ...
                memory_mask = memory_mask.flatten(
                    0, 1
                )  # (bs*n_heads, nq, H*W)
            if self.training:
                assert (
            ...
            if self.box_refine:
                ...
                if box_head_trk is None:
                    ...
                    delta_unsig = torch.cat(
                        [delta_unsig_det, delta_unsig_trk], dim=0
                    )
        ...
        return (
            ...
            (
                ...
                and is_instance_prompt is False
                else None
            ),
            presence_feats,
        )
...
class TransformerEncoderCrossAttention(nn.Module):
    def __init__(
        ...
        for i, remove_cross_attention in enumerate(
            self.remove_cross_attention_layers
        ):
    ...
    def forward_pre(
        ...
        if self.cross_attention_first:
            tgt = self._forward_ca(
                tgt, memory, query_pos, pos, num_k_exclude_rope
            )
