...
def main(args) -> None:
    ...
    if submitit_conf.use_cluster:
        ...
        submitit_conf.qos = (
            ...
            else submitit_conf.get("qos", None)
        )
        job_kwargs = {
        ...
        if (
            ...
            for job, runner, job_cfg, job_cfg_resolved in jobs_runners_configs:
                ...
                job_array_config_resolved_file = os.path.join(
                    job_array_config_dir,
                    "{}.config_resolved.yaml".format(job.job_id),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--partition", type=str, default=None, help="SLURM partition"
    )
    parser.add_argument(
        "--account", type=str, default=None, help="SLURM account"
    )
    ...
    parser.add_argument(
        "--num-nodes", type=int, default=None, help="Number of nodes"
    )
    args = parser.parse_args()
    args.use_cluster = (
        bool(args.use_cluster) if args.use_cluster is not None else None
    )
    register_omegaconf_resolvers()
    main(args)
