...
def all_gather_via_filesys(
    data, filesys_save_dir=None, gather_to_rank_0_only=False
):
    """
    ...
    assert not os.path.exists(
        save_data_path
    ), f"{save_data_path} already exists"
    ...
    if rank_save == 0 or not gather_to_rank_0_only:
        for rank_load in range(world_size):
            load_data_filename = (
                f"data_to_gather_{timestamp}_{salt}_{rank_load}.pkl"
            )
            load_data_path = os.path.join(save_dir, load_data_filename)
            assert os.path.exists(
                load_data_path
            ), f"cannot read {save_data_path}"
...
def all_gather(
    data, force_cpu=False, force_filesys=False, filesys_save_dir=None
):
    """
...
def convert_to_distributed_tensor(
    tensor: torch.Tensor,
...
def broadcast_object(
    obj: Any, src: int = _PRIMARY_RANK, use_disk: bool = True
) -> Any:
