...
def instance_masks_to_semantic_masks(
    ...
    return torch.stack(
        [torch.any(masks, dim=0) for masks in masks_per_query], dim=0
    )
...
def dice_loss(
    inputs, targets, num_boxes, loss_on_multimask=False, reduce=True
):
    """
    ...
    try:
        loss = _dice_loss(
            inputs, targets, num_boxes, loss_on_multimask, reduce
        )
...
def sigmoid_focal_loss(
    ...
    if loss_on_multimask:
        ...
        assert loss.dim() == 4
        return (
            loss.flatten(2).mean(-1) / num_boxes
...
class IABCEMdetr(LossWithWeights):
    def __init__(
        ...
        self.use_separate_loss_for_det_and_trk = (
            use_separate_loss_for_det_and_trk
        )
        if num_det_queries is not None:
            logging.warning(
                "note: it's not needed to set num_det_queries anymore"
            )
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        assert (
            len(outputs["pred_logits"].shape) > 2
        ), "Incorrect predicted logits shape"
        assert (
        ...
        with torch.no_grad():
            ...
            src_boxes_xyxy = outputs["pred_boxes_xyxy"][
                (indices[0], indices[1])
            ]
            target_boxes_giou = (
            ...
            t = prob[(indices[0], indices[1])] ** self.alpha * iou ** (
                1 - self.alpha
            )
        ...
        if self.use_presence:
            ...
            keep_loss = (
                gt_padded_is_visible.sum(dim=-1)[..., None] != 0
            ).float()
            ...
            if self.use_presence_semgseg:
                ...
                presence_logits = outputs["presence_logit_dec"].view_as(
                    keep_loss
                )
        ...
        if self.weak_loss:
            ...
            loss_mask = (
                (~targets["is_exhaustive"]).view(-1, 1).expand_as(loss_bce)
            )
            ...
            if self.use_separate_loss_for_det_and_trk:
                ...
                if self.no_loss_for_fp_propagation:
                    ...
                    loss_scales *= (
                        is_exhaustive | is_original_queries
                    ).float()

                loss_bce = loss_bce * loss_scales

            if (
                self.pad_n_queries is None
                or loss_bce.size(1) >= self.pad_n_queries
            ):
                ...
                loss_bce = loss_bce.sum() / (
                    self.pad_n_queries * loss_bce.size(0)
                )
...
class Masks(LossWithWeights):
    def __init__(
        ...
        if compute_aux:
            warnings.warn(
                "Masks loss usually shouldn't be applied to aux outputs"
            )
    ...
    def _sampled_loss(self, src_masks, target_masks, num_boxes):
        ...
        losses = {
            ...
            "loss_dice": dice_loss(
                sampled_src_masks, sampled_target_masks, num_boxes
            ),
        }
...
def segment_miou(source, target):
    ...
    ), "The two masks must have the same shape"
...
class SemanticSegCriterion(LossWithWeights):
    ...
    def get_loss(self, out_dict, targets):
        ...
        miou = segment_miou(
            outputs.sigmoid().squeeze(1) > 0.5, semantic_targets
        )
        ...
        if self.presence_head:
            ...
            if self.presence_loss:
                ...
                presence_acc = (
                    (
                        (presence_logit.flatten().sigmoid() > 0.5)
                        == presence_target
                    )
                    .float()
                    .mean()
...
class Det2TrkAssoc(LossWithWeights):
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        ...
        det2trk_assoc_labels = -torch.ones(
            B, Q_det, dtype=torch.long, device=device
        )
        ...
        det_is_same_obj_id_as_trk = (
            ...
            & (
                matched_obj_ids_det[:, :, None]
                == matched_obj_ids_trk[:, None, :]
            )
        )
        batch_idx, det_idx, trk_idx = det_is_same_obj_id_as_trk.nonzero(
            as_tuple=True
        )
        ...
        det_is_new_obj = det_is_matched_to_gt & ~det_is_same_obj_id_as_trk.any(
            dim=-1
        )
        ...
        if self.use_fp_loss:
            det_is_above_thresh = (
                outputs["pred_logits"][:, :Q_det].squeeze(2) > 0
            )
        ...
        frame_has_valid_trk = trk_is_matched_to_gt.any(
            dim=-1, keepdims=True
        )  # (B, 1)
...
class TrackingByDetectionAssoc(LossWithWeights):
    def __init__(self, weight_dict):
        super().__init__(
            weight_dict, compute_aux=False, supports_o2m_loss=False
        )
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        ...
        det2trk_assoc_logits = outputs[
            "det2trk_assoc_logits"
        ]  # (B, Q_det+1, Q_trk+1)
        ...
        torch._assert_async(
            torch.all(det_is_same_obj_id_as_trk.sum(dim=2) <= 1)
        )
        torch._assert_async(
            torch.all(det_is_same_obj_id_as_trk.sum(dim=1) <= 1)
        )
        batch_idx, det_idx, trk_idx = det_is_same_obj_id_as_trk.nonzero(
            as_tuple=True
        )
        ...
        det2trk_assoc_labels[det_is_new_obj] = (
            Q_trk  # "Q_trk" label is "new-object"
        )

        # softmax cross-entropy loss for detection-to-tracking association
        loss_det2trk_assoc = F.cross_entropy(
            input=det2trk_assoc_logits[:, :-1].flatten(
                0, 1
            ),  # (B*Q_det, Q_trk+1)
            target=det2trk_assoc_labels.flatten(0, 1),  # (B*Q_det)
            ignore_index=-1,
        ...
        trk2det_assoc_labels = -torch.ones(
            B, Q_trk, dtype=torch.long, device=device
        )
        ...
        trk_is_occluded = (
            trk_is_matched_to_gt & ~det_is_same_obj_id_as_trk.any(dim=1)
        )
        trk2det_assoc_labels[trk_is_occluded] = (
            Q_det  # "Q_det" label is "occluded"
        )

        # softmax cross-entropy loss for tracking-to-detection association
        loss_trk2det_assoc = F.cross_entropy(
            input=trk2det_assoc_logits[:, :-1].flatten(
                0, 1
            ),  # (B*Q_trk, Q_det+1)
...
def _keep_only_trk_queries_in_match_inds(inds, Q_det):
    ...
    if batch_idx.numel() == 0:
        return (
            ...
            src_idx,
            tgt_idx,
        )  # empty indices, nothing to filter
