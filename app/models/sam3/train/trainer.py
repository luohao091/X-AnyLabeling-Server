...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        self.optim_conf = (
            OptimConf(**optim) if optim is not None else OptimConf()
        )
    ...
    def _setup_torch_dist_and_backend(
    ...
    def _setup_device(self, accelerator):
        self.local_rank, self.distributed_rank = (
            get_machine_local_and_dist_rank()
        )
        if accelerator == "cuda":
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        for ckpt_name in checkpoint_names:
            checkpoint_paths.append(
                os.path.join(checkpoint_folder, f"{ckpt_name}.pt")
            )
    ...
    def _init_model_state(self):
        ...
        allow_init_skip_parameters = (
            self.checkpoint_conf.initialize_after_preemption
        )
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        if "train_dataset" in checkpoint and self.train_dataset is not None:
            self.train_dataset.load_checkpoint_state(
                checkpoint["train_dataset"]
            )
    ...
    def run_train(self):
        while self.epoch < self.max_epochs:
            ...
            if self.distributed_rank == 0:
                with g_pathmgr.open(
                    os.path.join(
                        self.logging_conf.log_dir, "train_stats.json"
                    ),
                    "a",
                ) as f:
            ...
            if self.is_intermediate_val_epoch(self.epoch):
                ...
                if (
                    ...
                    and self.empty_gpu_mem_cache_after_eval
                ):
    ...
    def val_epoch(self, val_loader, phase):
        ...
        loss_mts = OrderedDict(
            [
                (name, AverageMeter(name, self.device, ":.2e"))
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                # Log schedulers
                if data_iter % self.logging_conf.log_scalar_frequency == 0:
                    for j, param_group in enumerate(
                ...
                if data_iter % self.logging_conf.log_scalar_frequency == 0:
                    ...
                    for progress_meter in progress.meters:
                        self.logger.log(
                            os.path.join(
                                "Step_Stats", phase, progress_meter.name
                            ),
                            progress_meter.val,
                            self.steps[phase],
                        )
        ...
        self.est_epoch_time[Phase.TRAIN] = (
            batch_time_meter.avg * iters_per_epoch
        )
        ...
        for i, chunked_batch in enumerate(batch):
            ...
            with ddp_context:
                ...
                if not math.isfinite(loss.item()):
                    error_msg = (
                        f"Loss is {loss.item()}, attempting to stop training"
                    )
    ...
    def _log_meters_and_save_best_ckpts(self, phases: List[str]):
        ...
        for key, meter in self._get_meters(phases).items():
            ...
            for meter_subkey, meter_value in meter_output.items():
                out_dict[os.path.join("Meters_train", key, meter_subkey)] = (
                    meter_value
                )
                ...
                if (
                    tracked_meter_key not in self.best_meter_values
                    or is_better_check(
                        meter_value,
                    ...
                    if (
                        ...
                        checkpoint_save_keys.append(
                            tracked_meter_key.replace("/", "_")
                        )
    ...
    def _log_timers(self, phase):
        ...
        val_epochs_remaining = sum(
            n % self.val_epoch_freq == 0
            for n in range(self.epoch, self.max_epochs)
        )
        ...
        logging.info(
            f"Estimated time remaining: {human_readable_time(time_remaining)}"
        )
    ...
    def _setup_components(self):
        ...
        if self.loss_conf:
            self.loss = {
                ...
                for (key, el) in instantiate(
                    self.loss_conf, _convert_="all"
                ).items()
            }
        ...
        self.gradient_clipper = (
            instantiate(self.optim_conf.gradient_clip)
            if self.optim_conf
            else None
        )
        self.gradient_logger = (
            instantiate(self.optim_conf.gradient_logger)
            if self.optim_conf
            else None
        )
...
def print_model_summary(model: torch.nn.Module, log_dir: str = ""):
    ...
    logging.info(
        f"\tTotal parameters {get_human_readable_count(total_parameters)}"
    )
    logging.info(
