...
class YouTubeVIS(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            "TRACKERS_FOLDER": os.path.join(
                code_path, "data/trackers/youtube_vis/"
            ),
            # Trackers location
    ...
    def __init__(self, config=None):
        ...
        if self.config["GT_JSON_OBJECT"] is not None:
            ...
            gt_dir_files = [
                file
                for file in os.listdir(self.gt_fol)
        ...
        self.class_name_to_class_id = {
            k: v
            for k, v in cls_name_to_cls_id_map.items()
            if k in self.class_list
        }

        # Get sequences to eval and check gt files exist
        self.seq_list = [
            vid["file_names"][0].split("/")[0]
        ...
        if self.config["TRACKER_JSON_OBJECT"] is not None:
            ...
            for tracker in self.tracker_list:
                ...
                with open(
                    os.path.join(tracker_dir_path, tr_dir_files[0])
                ) as f:
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        all_cls_ids = {
            self.class_name_to_class_id[cls] for cls in self.class_list
        }
        classes_to_tracks = {
        ...
        raw_data["classes_to_tracks"] = {
            cls: [
                {
                    i: track[result_key][i]
                    for i in range(len(track[result_key]))
                }
                for track in tracks
            ]
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        data["gt_track_iscrowd"] = raw_data["classes_to_gt_track_iscrowd"][
            cls_id
        ]
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        if self.iou_type == "segm":
            ...
            tracker_dets_t = np.array(
                tracker_dets_t, dtype=np.float32
            ).reshape(-1, 4)
            ...
            for track in self.gt_data["annotations"]:
                ...
                for i, seg in enumerate(track["segmentations"]):
                    if seg is not None and isinstance(seg["counts"], list):
                        track["segmentations"][i] = mask_utils.frPyObjects(
                            seg, h, w
                        )
    ...
    def _get_tracker_seq_tracks(self, tracker, seq_id):
        ...
        tracks = [
            ann
            for ann in self.tracker_data[tracker]
            if ann["video_id"] == seq_id
        ]
        for track in tracks:
