...
class TAO_OW(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self._merge_categories(
            self.gt_data["annotations"] + self.gt_data["tracks"]
        )
        ...
        self.videos_to_gt_tracks, self.videos_to_gt_images = (
            self._compute_vid_mappings(self.gt_data["annotations"])
        )
        ...
        self.seq_to_images_to_timestep = (
            self._compute_image_to_timestep_mappings()
        )
        self.seq_to_classes = {
            vid["id"]: {
                "pos_cat_ids": list(
                ...
                "not_exhaustively_labeled_cat_ids": vid[
        ...
        # Get classes to eval
        considered_vid_ids = [
            self.seq_name_to_seq_id[vid] for vid in self.seq_list
        ]
        seen_cats = set(
            [
                cat_id
        ...
        for tracker in self.tracker_list:
            ...
            self.tracker_data[tracker][
                "vids_to_tracks"
            ] = curr_videos_to_tracker_tracks
            self.tracker_data[tracker][
                "vids_to_images"
            ] = curr_videos_to_tracker_images
        ...
        if not is_gt:
            raw_data["classes_to_dt_track_scores"] = {
                cls: np.array(
                    [
                        np.mean(
                            [float(x["score"]) for x in track["annotations"]]
                        )
                        for track in tracks
                    ]
                )
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        is_not_exhaustively_labeled = (
            cls_id in raw_data["not_exhaustively_labeled_cls"]
        )
        ...
        for t in range(raw_data["num_timesteps"]):
            ...
            if gt_ids.shape[0] > 0 and tracker_ids.shape[0] > 0:
                ...
                matching_scores[
                    matching_scores < 0.5 - np.finfo("float").eps
                ] = 0
        ...
        data["gt_track_lengths"] = raw_data["classes_to_gt_track_lengths"][
            cls_id
        ]
        ...
        data["dt_track_lengths"] = raw_data["classes_to_dt_track_lengths"][
            cls_id
        ]
        data["dt_track_areas"] = raw_data["classes_to_dt_track_areas"][cls_id]
        data["dt_track_scores"] = raw_data["classes_to_dt_track_scores"][
            cls_id
        ]
        ...
        if data["dt_tracks"]:
            ...
            data["dt_track_lengths"] = [
                data["dt_track_lengths"][i] for i in idx
            ]
    ...
    def _compute_vid_mappings(self, annotations):
        ...
        for vid, tracks in vids_to_tracks.items():
            for track in tracks:
                ...
                track["area"] = sum(
                    x["area"] for x in track["annotations"]
