...
class RexOmniWrapper:
    ...
    def _inference_batch(
        ...
        **kwargs,
        ...
        if self.model_type == "vllm":
            ...
            batch_outputs, batch_generation_info = (
                self._generate_transformers_batch(batch_messages, images)
            )
    ...
    def _generate_prompt(
        ...
        if task == TaskType.VISUAL_PROMPTING:
            ...
            return task_config.prompt_template.format(
                visual_prompt=visual_prompt_json
            )
            ...
            if task_config.requires_categories and categories is None:
                raise ValueError(
                    f"Categories are required for {task.value} task"
                )
    ...
    def _generate_vllm_batch(
        ...
        for output in outputs:
            ...
            tokens_per_second = (
                num_output_tokens / generation_time
    ...
    def _generate_transformers(self, messages: List[Dict]) -> Tuple[str, Dict]:
        ...
        generation_kwargs = {
            ...
            "top_k": self.top_k,
            "repetition_penalty": self.repetition_penalty,
            "do_sample": self.temperature
            > 0,  # Enable sampling if temperature > 0
